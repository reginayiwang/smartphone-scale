...
def evaluate(food,train_data,test_data,models=[]):
    ...
    for mod in models:
        ...
        # pca = PCA(n_components=6).fit(train_data[0])
        # train_data[0] = pca.transform(train_data[0])
        # test_data[0] = pca.transform(test_data[0])
...
def make_feats(df):
    ...
    for _,row in df.iterrows():
        if row['weight'] > mu + 4*std: continue 
        # if row['weight'] < mu - 2*std: continue
        ...
        # feats.append(np.concatenate((fin,hmag,hfre),axis=0))
        feats.append(hmag)
...
def prep_data(path='data/regina_03-29-22'):
    df = pd.DataFrame.from_dict(signals.parse_folder(path,mode='per'))
...
def make_og_feats(df,prune_outliers=True):
    ...
    mu,std = df['weight'].mean(),df['weight'].std()
    for _,row in df.iterrows():
        if prune_outliers and row['weight'] > mu + 4*std: continue 
...
def prep_og_data(path='data/all_data'):
    df = pd.DataFrame.from_dict(signals.parse_folder(path, mode='sub', OG=True))
...
def main(args):
    ...
    # models = [LinearRegression(),LassoLars(),Ridge(),ElasticNet(), RFE(ElasticNet(),n_features_to_select=3, step=1), RFECV(ElasticNetCV(),min_features_to_select=1,step=1,cv=8)]

    names = ['LinReg','ElasticNet','RFE(LR,n=1)','RFE(EN,n=1)','RFE(EN,n=2)','RFE(EN,n=3)','RFE(EN,n=6)','RFECV(ENCV)']
    models = [LinearRegression(),ElasticNet(),RFE(LinearRegression,n_features_to_select=1),RFE(ElasticNet(),n_features_to_select=1, step=1), RFE(ElasticNet(),n_features_to_select=2, step=1), RFE(ElasticNet(),n_features_to_select=6, step=1), RFE(ElasticNet(),n_features_to_select=6, step=1), RFECV(ElasticNetCV(),min_features_to_select=1,step=1,cv=5)]
    ...
    if args.vibroscale == False: 
        ...
        for food in train_data.keys():
            count,minw,maxw = len(train_data['All Food'][1]),train_data['All Food'][1].min(),train_data['All Food'][1].max()
            results = evaluate(food,train_data['All Food'],test_data[food],list(zip(names,models)))
...
if __name__=='__main__':
    ...
    parser.add_argument('--test_data',default='DataProcessing/data/test')
    parser.add_argument('-o','--output_file',default='DataProcessing/hmagper.csv')
